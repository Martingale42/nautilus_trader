...
class MT5InstrumentProvider(InstrumentProvider):
    """
    Provides Nautilus instrument definitions from MetaTrader 5.
    ...
    async def load_all_async(self, filters: dict | None = None) -> None:
        ...
        filters : dict, optional
            Not implemented for MT5 (all symbols are loaded).

        """
        # Query MT5 for all available symbols via ZeroMQ
        # The Rust client will send a SYMBOLS query to MT5 and return parsed instruments
        ...
        self._log.info(f"Loaded {len(instruments)} instruments from MT5")
    ...
    async def load_ids_async(
        ...
        """
        Load specific instruments by ID from MT5.
        ...
        # MT5 doesn't have per-instrument queries, so we load all and filter
        pyo3_instruments = await self._client.request_instruments()
        self._instruments_pyo3 = pyo3_instruments
        ...
        for instrument in instruments:
            if instrument.id not in instrument_ids:
                continue  # Filter out unwanted instruments
            self.add(instrument=instrument)

        self._log.info(f"Loaded {len(self._instruments)} instruments from MT5")
    ...
    async def load_async(
        ...
        """
        Load a single instrument by ID from MT5.
